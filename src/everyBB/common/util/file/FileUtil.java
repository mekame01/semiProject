...
public class FileUtil {
	//1. 유니크한 파일 이름을 생성해서 저장 (renameFileName 생성)
	//2. 저장 경로를 웹어플리케이션 외부로 지정
	//		저장 경로 + 연/월/일 폴더에 파일을 저장
	//3. 파라미터 데이터와 파일 메타 정보를 Map에 저장해 반환
	//4. 파일 삭제 메서드
		...
		try {
			...
			while((part = mp.readNextPart()) != null) {
				if(part.isFile()) {
					...
					//paramPart에 저장되어 있는 파라미터 값을 받아서
					//key : value형태로 저장, value는 list 타입
	...
	*/
	...
	private List<String> getParamValue(ParamPart params) throws UnsupportedEncodingException{
		...
		//이전에 같은 파라미터 이름으로 저장된 파라미터가 없다면
